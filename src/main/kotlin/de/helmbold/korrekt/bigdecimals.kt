...
fun Candidate<BigDecimal>.bePositive(): Candidate<BigDecimal> {
    assert(value >= BigDecimal.ZERO) { "$value is not positive." }
    return this
}

fun Candidate<BigDecimal>.notBePositive(): Candidate<BigDecimal> {
    assert(value < BigDecimal.ZERO) { "$value is positive." }
    return this
}

fun Candidate<BigDecimal>.beNegative(): Candidate<BigDecimal> {
    assert(value < BigDecimal.ZERO) { "$value is not negative." }
    return this
}

fun Candidate<BigDecimal>.notBeNegative(): Candidate<BigDecimal> {
    assert(value >= BigDecimal.ZERO) { "$value is negative." }
    return this
}

fun Candidate<BigDecimal>.beZero(): Candidate<BigDecimal> {
    assert(value == BigDecimal.ZERO) { "$value is not 0." }
    return this
}
