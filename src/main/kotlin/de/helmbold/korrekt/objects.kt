...
fun <T> Candidate<T>.beInstanceOf(expectedClass: Class<*>): Candidate<T> {
    assert(expectedClass.isInstance(value)) {
        "'$value' was no instance of '$expectedClass'."
    }
    return this
}

fun <T> Candidate<T>.beNoInstanceOf(notExpectedClass: Class<*>): Candidate<T> {
    assert(!notExpectedClass.isInstance(value)) {
        "'$value' was instance of '$notExpectedClass':"
    }
    return this
}
...
fun <T> Candidate<T>.beNoInstanceOf(notExpectedClass: KClass<*>): Candidate<T> {
    assert(!notExpectedClass.isInstance(value)) {
        "'$value' was no instance of '$notExpectedClass'."
    }
    return this
}
...
fun <T> Candidate<T>.notBeIn(values: Iterable<T>): Candidate<T> {
    assert(!values.contains(value)) { "'$value' was not in '$values'."}
    return this
}

fun <T> Candidate<T>.notBeIn(vararg values: T): Candidate<T> {
    this.notBeIn(values.asIterable())
    return this
}